...
class Scanner
{
    ...
    public function scan()
    {
        ...
        while (true) {
            ...
            if ($mixedContent) {
                ...
                foreach ($mixedContent as $url) {
                    ...
                    echo '<div class="detectedMixedContent"> Mixed content détécté : ' . $url . ' sur la page :'. $curPageUrl.' </div>';
                    
                }
            } else { // No mixed content
                $this->logger->info(sprintf('%05d', $curPageIndex).' - '.$curPageUrl);
                //echo sprintf('%05d', $curPageIndex).' - '.$curPageUrl. ' <br>';
            }
        ...
        echo '<span class="resultsScan"> Scanned '.count($this->pages).' pages for Mixed Content'. '</span> <br>';
    }
